...
`tools/list` at `https://gateway.pipeworx.io/agify/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/predict_age \
  -H 'Content-Type: application/json' \
  -d '{"name":"John"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/predict_age`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
